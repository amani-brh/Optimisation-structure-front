...
export default function CalculPage() {
  ...
  const fy = FY_MAP[nuance];

  // Déplacements — limits derived from geometry
  const limHoriz = (H * 100) / 150;          // H/150 in cm
  const limVert  = (L * 100) / 250;          // L/250 in cm

  const handleNuance = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const next = e.target.value;
    setNuance(next);
    setChecks(buildInitialChecks(FY_MAP[next]));
  };

  const run = () => {
    setRunning(true);
    setTimeout(() => {
      setChecks(buildResults());
      setRunning(false);
    }, 1800);
  };

  const ran = checks[0].status !== '—';
  ...
  return (
    <div className="mod-page">
      ...
      {/* ── Inputs card ── */}
      <div className="mod-card" style={{ marginBottom: '1rem' }}>
        ...
        <div className="mod-grid-2">
          <div>
            <div className="mod-field">
              ...
              <select className="mod-field-select">
                <option>HEA 200</option><option>HEA 220</option>
                <option>HEA 240</option><option>HEA 260</option>
                <option>HEA 280</option><option>HEB 200</option>
              </select>
            </div>
            <div className="mod-field">
              <label className="mod-field-label">Traverse — Profil IPE/HEA</label>
              <select className="mod-field-select">
                <option>IPE 270</option><option>IPE 300</option>
                <option>IPE 330</option><option>IPE 360</option>
          ...
          <div>
            <div className="mod-field">
              ...
              <select className="mod-field-select" value={nuance} onChange={handleNuance}>
                <option>S235</option>
                <option>S275</option>
                <option>S355</option>
              </select>
              <div className="mod-field-unit">fy = {fy} MPa</div>
            </div>
            <div className="mod-field">
      ...
      <div className="mod-grid-3">
        ...
        {/* Card 2 — Contraintes */}
        <div className="mod-card">
          <div className="mod-card-title"><span className="mod-card-dot" />Contraintes</div>
          <table className="mod-table">
            <thead>
              <tr>
                <th>Vérification</th>
            ...
            <tbody>
              <tr>
                ...
                <td className="mod-val-accent">{fy} MPa</td>
                <td>fy </td>
              </tr>
            </tbody>
          </table>
        ...
        {/* Card 3 — Déplacements */}
        <div className="mod-card">
          <div className="mod-card-title"><span className="mod-card-dot" />Déplacements</div>
          <table className="mod-table">
            <thead>
              <tr>
                <th>Vérification</th>
            ...
            <tbody>
              <tr>
                <td>Horizontal — tête poteau</td>
                <td className="mod-val-accent"> {limHoriz.toFixed(1)} cm</td>
                <td>H/150 </td>
              </tr>
              <tr>
                <td>Vertical — faîtage</td>
                <td className="mod-val-accent"> {limVert.toFixed(1)} cm</td>
                <td>L/250 </td>
              </tr>
            </tbody>
          </table>
      ...
      {/* ── Summary metrics ── */}
      {ran && (
        <div className="mod-result" style={{ marginTop: '1rem' }}>
          ...
          <div className="mod-grid-4">
            ...
            <div className="mod-metric">
              ...
              <div className="mod-metric-value" style={{ color: 'var(--em)' }}>{fy} MPa</div>
              <div className="mod-metric-unit">≤ {fy} MPa ✓</div>
            </div>
            <div className="mod-metric">
